...
(function () {
  ...
  // The leader is the first LIVING member in command order. The OPERATOR dying
  // ends the run (checkEnd), so in practice the leader is the OPERATOR.
  const lead = () => living()[0];
  const memberAt = (c, r) => living().find(m => m.c === c && m.r === r);
  // Foes hunt the NEAREST living member; ties go to the frailest. This is the
  // whole point of splitting the token: CINIS lagging the trail is now a
  // choice foes get to punish.
  ...
  function recomputeFOV() {
    ...
    // Seeded from EVERY living member: a daemon at the back of the trail must
    // not stand in its own fog.
    visible = new Set(living().map(m => key(m.c, m.r)));
    let wave = living().map(m => [m.c, m.r]);
  ...
  function collectDrops() {
    // Leader-only pickup: followers walk the same path, so per-follower
    // pickup would add nothing but double-fire bugs.
    const L = lead();
    const here = state.drops.filter(d => d.c === L.c && d.r === L.r);
  ...
  function strikeMember(foe) {
    // Melee reaches whoever is actually beside the foe — no more hitting a
    // member three tiles back through the shared token.
    const targets = living().filter(m => adjacent(foe, m));
    if (!targets.length) return;
    const t = targets[Math.floor(Math.random() * targets.length)];
    const dmg = roll(foe.atk, defOf(t));
    t.hp -= dmg;
    float(t.c, t.r, '-' + dmg, 'f-hurt', t.name);
    burst(t.c, t.r, 'foe');
    if (t.hp <= 0) {
      t.hp = 0;
  ...
  // The side-effecting half: wake check and its log line. This is the ONLY
  // part the render path must never touch, which is why it is split from the
  // decision proper -- see decide().
  function foeIntent(foe, claimed) {
    if (foe.hp <= 0) return null;
    if (!foe.awake && living().some(m => cheb(foe, m) <= foe.aggro)) {
      foe.awake = true;
      log(`${foe.kind} WAKES.`, 'bad');
    }
    if (!foe.awake) return { kind: 'idle' };
    return decide(foe, claimed);
  }

  // The brain proper: pure. No waking, no logging, no writes of any kind, so
  // draw() can call it. previewIntent() runs THIS, which is the whole point:
  // the indicator and the resolution now come out of one body of code.
  //
  // They used to be two. previewIntent held its own three-line heuristic
  // ("attack if adjacent, else walk straight at the nearest member") and it
  // drifted badly from what actually resolves: skirmishers were drawn
  ...
  function decide(foe, claimed) {
    ...
    const target = foeTarget(foe);
    if (adjacent(foe, target)) return { kind: 'attack' };
    return { kind: 'step', chase: true, ...stepToward(foe, claimTile(foe, claimed, target), true) };
  }

  // FLANK: each pursuer claims its own free orthogonal tile beside the member
  // it hunts, so foes envelop instead of queueing behind one another.
  // `claimed` is a per-round Set so no two foes chase the same tile.
  function claimTile(foe, claimed, target) {
    const opts = [[0, -1], [-1, 0], [0, 1], [1, 0]]
      .map(([dc, dr]) => ({ c: target.c + dc, r: target.r + dr }))
    ...
    if (!opts.length) return target;
    claimed.add(key(opts[0].c, opts[0].r));
    return opts[0];
  }

  // SKIRMISH: hangs back until its hunted member is pinned by another foe,
  // commits then; withdraws when badly hurt; fights only when cornered.
  function skirmishIntent(foe, claimed) {
    const t = foeTarget(foe);
    if (foe.hp < foe.vitae * 0.5) {
      const away = stepAway(foe, t);
    ...
    if (pinned) {
      ...
      return { kind: 'step', chase: true, ...stepToward(foe, claimTile(foe, claimed, t), true) };
    }
    if (cheb(foe, t) < 3) {
      const away = stepAway(foe, t);
      if (away) return { kind: 'step', ...away };
  ...
  // Step that opens distance from the hunted member. Null when boxed in.
  function stepAway(foe, t) {
    const opts = [[0, -1], [-1, 0], [0, 1], [1, 0]]
  ...
  // flank tiles first-come, so a fresh Set per foe would show every pursuer
  // converging on the same tile instead of enveloping.
  ...
  // back off below half VITAE or inside 3 tiles, and "CLOSING IN" on a foe
  // that is running away is the exact read the player must not be given.
  function withdrawing(foe, it) {
    if (!it || it.kind !== 'step' || (!it.dc && !it.dr)) return false;
    const t = foeTarget(foe);
    if (!t) return false;
    return cheb({ c: foe.c + it.dc, r: foe.r + it.dr }, t) > cheb(foe, t);
  }
  ...
  const noTarget = (m, op) => op.targets !== 'circle' && validTargets(op, m).length === 0;
  const usable = (m, op) => affordable(m, op) && !noTarget(m, op);
  ...
  // Range is measured from the CASTER's tile: a trailing CINIS reaches two
  // tiles less deep than the OPERATOR on point. Positioning is the build now.
  function validTargets(op, m) {
    if (op.targets === 'circle') return [];
    return liveFoes().filter(f => foeSeen(f) && cheb(f, m) <= op.range);
  }
  ...
  function applyOp(entry) {
    ...
    if (op.vitaeCost) {
      ...
      float(member.c, member.r, '-' + op.vitaeCost, 'f-hurt', member.name);
      burst(member.c, member.r, 'selfburn');
    }
  ...
  function resolveRound(move) {
    ...
    runPhases([
      ...
      foesStep && (() => {
        ...
        // speed 2 matches the party's stepsPerRound, so pursuit cannot be
        // kited forever. Skirmish retreats stay speed 1 -- catchable.
        for (const { foe, intent } of foeIntents) {
          if (!intent || !intent.chase || foe.hp <= 0) continue;
          const t = foeTarget(foe);
          if (!t || cheb(foe, t) <= 2) continue;
          const s = stepToward(foe, t);
  ...
  function chooseOp(slot) {
    ...
    if (!validTargets(op, m).length) { warn(`${op.name} HAS NO TARGET IN RANGE.`); return; }
  ...
  function confirmTarget() {
    ...
    const m = state.pending.member;
    const near = validTargets(state.pending.op, m)
      .slice().sort((a, b) => cheb(a, m) - cheb(b, m))[0];
    if (!near) return;
  ...
  function moveInput(dc, dr) {
    ...
    if (swap) {
      swap.c = L.c; swap.r = L.r;
      L.c = nc; L.r = nr;
    } else {
      const chain = living();
      let vc = L.c, vr = L.r;              // tile being vacated
      L.c = nc; L.r = nr;
      for (let i = 1; i < chain.length; i++) {
        const f = chain[i], pc = f.c, pr = f.r;
        f.c = vc; f.r = vr;
        vc = pc; vr = pr;
      }
  ...
  function actorNode(key, onTop) {
    ...
    if (!el) {
      ...
      // Party nodes are appended (onTop); foe nodes slide in beneath the
      // first party node so members always draw above foes.
      const anchor = Object.keys(actorNodes).find(k => k.startsWith('party-'));
      if (onTop || !anchor) actorsG.appendChild(el);
      else actorsG.insertBefore(el, actorNodes[anchor]);
      actorNodes[key] = el;
    }
    return el;
  }
  ...
  // Level bounds: a tile edge is a wall wherever the neighbour is not
  // walkable. Full-edge lines (inset 0) so adjacent segments join into one
  // continuous perimeter instead of reading as per-tile squares.
  function boundEdges(c, r) {
    const x = c * T, y = r * T;
    let g = '';
    const seg = (x1, y1, x2, y2) =>
      `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="var(--cyan)"
             stroke-width="1.4" opacity=".5"/>`;
    if (!walkable.has(key(c, r - 1))) g += seg(x, y, x + T, y);
    if (!walkable.has(key(c, r + 1))) g += seg(x, y + T, x + T, y + T);
    if (!walkable.has(key(c - 1, r))) g += seg(x, y, x, y + T);
    if (!walkable.has(key(c + 1, r))) g += seg(x + T, y, x + T, y + T);
    return g;
  }
  ...
  const floorLayer = () => F.tiles.map(([c, r]) => {
    ...
    // Invisible squares: seamless fill only, no per-tile strokes. The only
    // lines are the level bounds traced by boundEdges().
    const tile =
      `<polygon points="${plate(c, r, 0)}" fill="#05080E" opacity=".75"/>` + boundEdges(c, r);
  ...
  function aimLayer() {
    ...
    let g = '';
    for (const [c, r] of F.tiles) {
      if (!inSight(c, r) || cheb({ c, r }, state.pending.member) > op.range) continue;
      g += `<polygon points="${plate(c, r, 3)}" fill="var(--cyan)" opacity=".10"/>`;
    }
    for (const f of validTargets(op, state.pending.member)) {
  ...
  function stagedLayer() {
    ...
    return `<polygon points="${plate(c, r, 3)}" fill="var(--gold)" opacity=".14"/>
            ...
            <g transform="translate(${c * T},${r * T})" opacity=".38">${S[MEMBER_SPRITE[lead().name]]()}</g>`;
  }
  ...
  const tokenRect = () => tilePos(lead().c, lead().r);

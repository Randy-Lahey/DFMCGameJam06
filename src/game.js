...
(function () {
  ...
  function previewIntent(foe) {
    ...
    const t = foeTarget(foe);
    if (adjacent(foe, t)) return { kind: 'attack', target: t };
    return { kind: 'step', ...stepToward(foe, t) };
  }
  ...
  function intentLayer() {
    ...
    for (const f of liveFoes()) {
      ...
      if (!it) continue;
      if (it.kind === 'attack') {
        const k = key(it.target.c, it.target.r);
        if (struckTiles.has(k)) continue;
        struckTiles.add(k);
        g += `<polygon points="${plate(it.target.c, it.target.r, 5)}" fill="none"
